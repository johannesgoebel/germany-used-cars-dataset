...
point scaleX scaleY xyPoint =
    g [ class [ "point" ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
        ...
        , text_
            ...
            , y (Scale.convert scaleY xyPoint.y)
...
sortList : List Car -> List Int
sortList my_car =
    List.sort (mpgValues my_car)


scatterplot : String -> String -> XyData -> (Float, Float)-> Svg msg
scatterplot xname yname model gleichung =
    let
        ...
        wagon_filtered =
            List.filter (\x -> x.carType == Wagon) filteredCars
        
       

        {- Ende der Sortierung und Berechnung -}
    in
    svg [ viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        ...
        , g [ transform [ Translate padding padding ] ]
            [line 
            [x1 (Scale.convert xScaleLocal (2))
            , y1 (Scale.convert yScaleLocal ((Tuple.first gleichung) * 2 + (Tuple.second gleichung)))
            , x2 (Scale.convert xScaleLocal (4))
            , y2 (Scale.convert yScaleLocal ((Tuple.first gleichung) * 4 + (Tuple.second gleichung)))
...
getmpg2: List (Float, Float) -> List Float
getmpg2 l =
    List.map  (Tuple.first ) l
              
              
getmpg3: List (Float, Float) -> List Float
getmpg3 r =
    List.map   (Tuple.second ) r 
              
              
 

makeQuantiles : List Float -> Float -> Float
makeQuantiles list quant=
    Statistics.quantile quant list |> Maybe.withDefault 0
        
        
punkt: List (Float, Float) -> Float -> (Float, Float)
punkt liste quant = ((makeQuantiles  (getmpg2 liste) quant), (makeQuantiles  (getmpg3 liste) quant))
        
geradeBerechnen: (Float, Float) -> (Float, Float) -> (Float, Float)
geradeBerechnen p1 p2 =
    let
        steigung: Float
        steigung = ((Tuple.second p2) - (Tuple.second p1))/((Tuple.first p2) - ((Tuple.first p1)))
        verschiebung: Float
        verschiebung = (Tuple.second p2) - steigung  * (Tuple.first p2)
    in
        (steigung, verschiebung)

filterCar : Car -> Maybe Car
filterCar car =
    if
        car.cityMPG
            /= Nothing
...
filter_cars myCars =
    List.filterMap filterCar myCars


car2Point : Car -> Maybe Point
car2Point car =
    Maybe.map4
        (\cityMPG retailPrice dealerCost carLen ->
            Point
...
filterAndReduceCars my_cars =
    let
        filtered_cars =
            List.filterMap car2Point my_cars
    in
    XyData "cityMPG" "retailPrice" filtered_cars


tuple2Point : ( Float, Float ) -> Point
tuple2Point tuplecar =
    Point
        (" (" ++ String.fromFloat (Tuple.first tuplecar) ++ "," ++ String.fromFloat (Tuple.second tuplecar) ++ ")")
        (Tuple.first tuplecar)
        ( (Tuple.second tuplecar))


list2XyData : List ( Float, Float ) -> XyData
list2XyData my_cars =
    let
        mapped_Points =
            List.map tuple2Point my_cars
    in
    XyData "f-Value CityMPG" "Quantile CityMPG" mapped_Points


mpgValues : List Car -> List Int
mpgValues mpg =
    List.map (\x -> Maybe.withDefault 0 x.cityMPG) mpg


averageList : List Car -> Float
averageList average =
    toFloat (List.sum (mpgValues average)) / toFloat (List.length average)

normalize_values: List (Float, Int) -> List (Float, Float)
normalize_values aList =
    let    
          getmpg: List (Float, Int) -> List Float
          getmpg l=
              List.map  Tuple.second  l|> List.map toFloat 
              
              
          mu: Float
          mu = List.sum (getmpg aList) / (toFloat <| List.length aList)
          
          std: Float
          std = Statistics.deviation (getmpg aList) |> Maybe.withDefault 0 |> sqrt
          ...
          normalF: (Float, Int) -> Float
          normalF f=
              mu + std * (invNormalCdf ( Tuple.first f))
              
          insertQQs : ( Float, Int ) -> ( Float, Float )
          insertQQs xy = 
               ( (normalF xy), ( toFloat (Tuple.second xy)))
               
    in
        List.map insertQQs aList


normalize_values_f: List (Float, Float) -> List (Float, Float)
normalize_values_f aList =
    let    
          getmpg: List (Float, Float) -> List Float
          getmpg l=
          ...
          insertQQs : ( Float, Float ) -> ( Float, Float )
          insertQQs xy = 
               ( (normalF xy), ( Tuple.second xy))
               
    in
        List.map insertQQs aList
        

makeQqList : List Int -> List ( Float, Int ) -> List ( Float, Int )
makeQqList biggerList smallerList =
    let
        getQuantile : ( Float, Int ) -> ( Float, Int )
        getQuantile tuple_s =
            ( Statistics.quantile (Tuple.first tuple_s) (List.map toFloat biggerList) 
            |> Maybe.withDefault 0, Tuple.second tuple_s )
    in
    List.map getQuantile smallerList
...
main =
    let
        ...
        wagon_sorted =
            sortList wagon_filtered

        wagon_indexed =
            List.indexedMap Tuple.pair wagon_sorted

        wagon_fvalues =
            List.map (\x -> ( ((toFloat (Tuple.first x) + 1) - 0.5) / toFloat (List.length wagon_indexed), Tuple.second x )) wagon_indexed

        

        suv_filtered =
            List.filter (\x -> x.carType == Sports_Car) filteredCars

        suv_sorted =
            sortList suv_filtered
            
---  Power transformation 
        suv_log = 
            List.map toFloat suv_sorted |> List.map (logBase e)
---- 
        suv_indexed =
            List.indexedMap Tuple.pair suv_log
        
        
            
        suv_fValues =
            List.map (\x -> ( ((toFloat (Tuple.first x) + 1) - 0.5) / toFloat (List.length suv_indexed), Tuple.second x )) suv_indexed
        
         
        suv_fValues_normalized =
            normalize_values_f suv_fValues
           
        xy_suv =
            list2XyData suv_fValues_normalized

        combined_qqs =
            makeQqList suv_sorted wagon_fvalues
            
        gerade_tuple =
            (geradeBerechnen (punkt suv_fValues_normalized 0.25) (punkt suv_fValues_normalized 0.75))

    in
    Html.div []
        [ Html.p []
            [ Html.text ((String.fromFloat(Tuple.first gerade_tuple) ++ " leer "++ String.fromFloat(Tuple.second gerade_tuple)) ++ " " ++ (String.fromFloat(Tuple.first (punkt suv_fValues_normalized 0.25)) ++ " " ++ String.fromFloat(Tuple.second (punkt suv_fValues_normalized 0.25)))) 
            , Html.br [] []
            , Html.text ("Filtered Cars Wagon: " ++ String.fromInt (List.length wagon_filtered))
            , Html.br [] []
            , Html.text ("Filtered Cars SUV: " ++ String.fromInt (List.length suv_filtered))             
            ]       
        , scatterplot "f-values" "quantiles SUV" xy_suv gerade_tuple
        ]
